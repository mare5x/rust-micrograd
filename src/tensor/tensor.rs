...
/// # Notes
/// - This struct is wrapped into `Tensor` with `Rc<RefCell<_>>` because
/// we are building a computation graph/tree.
/// - For simplicity we store everything as a 2d matrix (because `ndarray::dot`
/// doesn't work for dimensions >= 3).
#[derive(Debug)]
...
/// `TensorData` wrapper that acts like a PyTorch Tensor.  
/// # Notes
/// - `Tensor::Clone` is used to make a clone of the pointer to the underlying data.
#[derive(Debug, Clone)]
...
/// Tensor operations supporting `backward`.
impl Tensor {
